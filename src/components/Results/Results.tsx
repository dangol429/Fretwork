...
function Playable({ voicing, caption, note, badge, capo = 0, playing, index }: PlayableProps) {
  return (
    <button
      ...
      aria-label={`Play ${playing}. ${caption}${note ? `. ${note}` : ""}. ${readShape(voicing, capo)}`}
    >
...
export function Results({ result, onSearch, onHome }: ResultsProps) {
  ...
  return (
    <main className="results">
      <header className="results__bar">
        <button type="button" className="results__home" onClick={onHome}>
          {SITE_NAME}
        </button>
      ...
      <div className="results__head" data-enter="head">
        ...
        <p className="results__spoken">{chord.spoken}</p>
